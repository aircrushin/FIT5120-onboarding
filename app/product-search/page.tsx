...
export default function ProductSearchPage() {
  ...
  const getTrustScoreComponent = (trustScore: number, trustLevel: string) => {
    ...
    return (
      <div
        className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border ${getScoreBg()}`}
      >
        {getScoreIcon()}
  ...
  const generateDecisionReasoning = (product: Product) => {
    const bannedIngredients = product.ingredients.filter(
      (ing) => ing.risk_type === "B"
    );
    const highRiskIngredients = product.ingredients.filter(
      (ing) => ing.risk_type === "H"
    );
    const lowRiskIngredients = product.ingredients.filter(
      (ing) => ing.risk_type === "L"
    );
    ...
    if (product.prod_status_type === "C") {
      ...
      if (bannedIngredients.length > 0) {
        reasons.push({
          ...
          description: `This product contains ${
            bannedIngredients.length
          } banned substance${
            bannedIngredients.length > 1 ? "s" : ""
          }: ${bannedIngredients
            .map((ing) => ing.name)
            .join(
              ", "
            )}. These ingredients are prohibited due to their potential health risks.`,
      ...
      if (highRiskIngredients.length > 0) {
        reasons.push({
          ...
          description: `Contains ${
            ...
            highRiskIngredients.length > 1 ? "s" : ""
          } that may cause adverse reactions in sensitive individuals.`,
      ...
      if (reasons.length === 0) {
        reasons.push({
          ...
          description:
            "This product was cancelled due to regulatory compliance issues, manufacturing problems, or other administrative reasons.",
          ingredients: [],
          icon: "ban",
        });
      }
      ...
      if (lowRiskIngredients.length > 0) {
        reasons.push({
          ...
          description: `This product contains ${
            lowRiskIngredients.length
          } low-risk ingredient${
            lowRiskIngredients.length > 1 ? "s" : ""
          } that are generally recognized as safe for cosmetic use.`,
      ...
      if (highRiskIngredients.length > 0) {
        reasons.push({
          ...
          description: `Contains ${highRiskIngredients.length} ingredient${
            highRiskIngredients.length > 1 ? "s" : ""
          } that require careful monitoring. While approved, these may cause reactions in sensitive individuals.`,
          ingredients: highRiskIngredients.map((ing) => ing.name),
          icon: "warning",
        });
      }
      ...
      return {
        ...
        primaryReason: reasons[0] || {
          ...
          description:
            "This product has passed all regulatory requirements and safety assessments.",
          ingredients: [],
          icon: "check",
        },
        allReasons: reasons,
  ...
  const getOverallSafetyStatus = (product: Product) => {
    ...
    const hasBannedIngredients = product.ingredients.some(
      (ing) => ing.risk_type === "B"
    );
    const hasHighRiskIngredients = product.ingredients.some(
      (ing) => ing.risk_type === "H"
    );
    ...
    if (hasBannedIngredients) {
      ...
      return {
        status: "caution",
        message:
          "This product contains high-risk ingredients. Use with caution.",
        trustScore: trustData.score,
        trustLevel: trustData.level,
        reasoning,
      };
              ...
              <>
                ...
                {searchResults.map((product) => {
                  ...
                  const hasRelevanceScore =
                    ...
                    (product as any).relevanceScore > 0;

                  return (
                    <div
                      key={product.prod_notif_no}
                      className="bg-white rounded-xl shadow-lg overflow-hidden border-l-4 border-l-gray-200 hover:shadow-xl transition-shadow duration-200"
                    >
                      ...
                      >
                        ...
                        <div
                          className={`h-2 ${
                            safetyStatus.trustScore >= 85
                              ? "bg-green-500"
                              : safetyStatus.trustScore >= 70
                              ? "bg-blue-500"
                              : safetyStatus.trustScore >= 50
                              ? "bg-yellow-500"
                              : safetyStatus.trustScore > 0
                              ? "bg-orange-500"
                              : "bg-red-500"
                          }`}
                        ></div>
                            ...
                            <div className="flex-1">
                              <div className="flex items-start justify-between mb-3">
                                ...
                                <div className="lg:hidden">
                                  {getTrustScoreComponent(
                                    safetyStatus.trustScore,
                                    safetyStatus.trustLevel
                                  )}
                                </div>
                              </div>
                        ...
                        <div className="p-6 bg-gray-50 border-t">
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <h4 className="text-lg font-semibold text-gray-900">
                              Why was this product{" "}
                              {safetyStatus.reasoning.decision}?
                            </h4>
                          </div>
                          ...
                          <div
                            className={`p-4 rounded-lg border-l-4 mb-4 ${
                              safetyStatus.reasoning.primaryReason.type ===
                              "critical"
                            ...
                            <div className="flex items-start gap-3">
                              <div
                                className={`p-1 rounded-full ${
                                  ...
                                  "critical"
                                    ? "bg-red-100"
                                    : safetyStatus.reasoning.primaryReason
                                        .type === "warning"
                                    ? "bg-orange-100"
                                    : safetyStatus.reasoning.primaryReason
                                        .type === "caution"
                              ...
                              >
                                {safetyStatus.reasoning.primaryReason.icon ===
                                  "ban" && (
                              ...
                              <div className="flex-1">
                                ...
                                {safetyStatus.reasoning.primaryReason
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {safetyStatus.reasoning.primaryReason.ingredients.map(
                                      (ingredient, idx) => (
                                        <span
                                          key={idx}
                          ...
                          {safetyStatus.reasoning.allReasons.length > 1 && (
                            <div className="space-y-3 mb-4">
                              {safetyStatus.reasoning.allReasons
                                .slice(1)
                                .map((reason, index) => (
                                  <div
                                    key={index}
                        ...
                        <div className="p-6 border-t bg-white">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <a
                              href={`/alternatives/${encodeURIComponent(
                                product.prod_notif_no
                              )}`}
                          ...
                          <p className="text-sm text-gray-500">
                            You will be redirected to a page with similar
                            approved products from trusted brands.
                          </p>
                        </div>
                          ...
                          <div className="space-y-3">
                            {product.ingredients
                              .sort((a, b) => {
                                ...
                                return (
                                  riskOrder[a.risk_type] -
                                  riskOrder[b.risk_type]
                                );
                              })
